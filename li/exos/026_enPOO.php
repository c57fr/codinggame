...
class Game
{
    ...
    {
        if (getenv("SOHEL")) $this->printState();
    ...
    {
        /** @var Vault|null $minVault */
        $minVault = null;
        foreach ($this->vaults as $vault) {
            if ($minVault == null and $vault->robber) {
                $minVault = $vault;
            } else if ($minVault && $vault->robber && $vault->unlockTime < $minVault->unlockTime) {
                $minVault = $vault;
            }
        }
    ...
    {
        foreach ($this->vaults as $vault) {
            if ($vault->is_done == false and $vault->robber == null) {
                $robber->vault = $vault;
                $vault->robber = $robber;
                break;
            }
